...
function parseDocument(input, { keep, quiet }) {
...
function getOutputStr(doc, { es6, node, pretty, yaml }) {
  if (yaml) return String(doc)
  const json = pretty
...
function main({ input, output, ...opt }) {
  if (input === '-' && process.stdin.isTTY) {
    yargs.showHelp()
    process.exit(1)
  }

  const doc = parseDocument(input, opt)
  const outputStr = getOutputStr(doc, opt)
